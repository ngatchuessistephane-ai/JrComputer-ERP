<div style="zoom:0.90;">
<style>
    /* ─── SHARED MODULE STYLES (identique) ─── */
...
<div class="data-card">
    <div class="table-responsive">
        <table class="data-table">
            <thead><tr><th>Référence</th><th>Fournisseur</th><th>Date</th><th>Livraison prévue</th><th>Total</th><th>Statut</th>
            @can('create purchase orders')
            <th>Actions</th>
            @endcan
        </tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td><div class="action-btns">
                        ...
                        <button wire:click="receiveOrder({{ $po->id }})" class="act-btn receive" onclick="return confirm('Réceptionner cette commande ?')"><i class="bi bi-arrow-down-circle"></i></button>
                        @endcan
                        @can('edit purchase orders')
                        <button wire:click="cancelOrder({{ $po->id }})" class="act-btn cancel" onclick="return confirm('Annuler cette commande ?')"><i class="bi bi-x-circle"></i></button>
                        @endcan
                        @endif</div></td>
                </tr>
                @empty
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form wire:submit.prevent="save">
                    ...
                    {{-- Ajout d'articles --}}
                    <div style="background:var(--bg-page);border-radius:11px;padding:16px 18px;margin-bottom:18px;border:1px solid var(--border-color);">
                        <div style="font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text-muted);margin-bottom:12px;"><i class="bi bi-box-seam me-1"></i> Ajouter un article</div>
                        <div class="row g-2 align-items-end">
                            <div class="col-md-5"><div class="fg" style="margin-bottom:0;"><label>Produit</label><select wire:model="selectedProduct" class="fc @error('selectedProduct') is-invalid @enderror"><option value="">— Sélectionner un produit —</option>@foreach($products as $p)<option value="{{ $p->id }}">{{ $p->name }} ({{ $p->reference }})</option>@endforeach</select>@error('selectedProduct')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div></div>
                            <div class="col-md-2"><div class="fg" style="margin-bottom:0;"><label>Quantité</label><input type="number" wire:model="quantityOrdered" class="fc @error('quantityOrdered') is-invalid @enderror" min="1">@error('quantityOrdered')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div></div>
                            <div class="col-md-3"><div class="fg" style="margin-bottom:0;"><label>Prix unitaire (FCFA)</label><input type="number" step="0.01" wire:model="unitPrice" class="fc @error('unitPrice') is-invalid @enderror">@error('unitPrice')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div></div>
                            <div class="col-md-2"><button type="button" wire:click="addItem" class="btn-brand" style="width:100%;justify-content:center;"><i class="bi bi-plus-lg"></i> Ajouter</button></div>
                        </div>
                    </div>
...
<div class="modal fade modal-jr" id="exportFiltersModal" tabindex="-1" wire:ignore.self>
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row g-3">
                    <div class="col-md-6"><div class="fg"><label>Date commande (début)</label><input type="date" wire:model="filter_date_from" class="fc"></div></div>
                    <div class="col-md-6"><div class="fg"><label>Date commande (fin)</label><input type="date" wire:model="filter_date_to" class="fc"></div></div>
                    ...
                    <div class="col-md-6"><div class="fg"><label>Fournisseur</label>
                        <select wire:model="filter_supplier_id" class="fc">
                            <option value="">-- Tous --</option>
                    ...
                    <div class="col-md-6"><div class="fg"><label>Total min (FCFA)</label><input type="number" wire:model="filter_total_min" class="fc" placeholder="0"></div></div>
                    <div class="col-md-6"><div class="fg"><label>Total max (FCFA)</label><input type="number" wire:model="filter_total_max" class="fc" placeholder="Illimité"></div></div>
                </div>
            </div>
